...
impl Expression {
    fn new_const(val: f64, start: usize, end: usize) -> Expression {
        let data = ExpressionData::Constant(val);
        Expression { data, start, end }
    }

    fn new_id(name: char, start: usize, end: usize) -> Expression {
        let data = ExpressionData::Identifier(name);
        Expression { data, start, end }
    }

    fn new_op(lhs: Expression, op: Operation, rhs: Expression) -> Expression {
        let start = lhs.start;
        let end = rhs.end;
...
fn parse_tokens(tokens: &[Token], start: usize, end: usize) -> Result<Expression, Error> {
    ...
    let (mut curr_lhs, mut i) = match tokens[start].token_type {
        ...
        TokenType::OpenParen => {
            ...
            let inner_expr = parse_tokens(tokens, start + 1, end_paren - 1)?;
    ...
    while i < end {
        ...
        let curr_rhs = match tokens[i].token_type {
            ...
            TokenType::OpenParen => {
                ...
                let old_i = i;
                i = end_paren + 1;
                let inner_expr = parse_tokens(tokens, old_i + 1, end_paren - 1)?;
